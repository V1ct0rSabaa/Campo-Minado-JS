...
function tempo_jogo(parar_cronometro){
...
function set_campo(quantidade_linha, quantidade_coluna, quantidade_mina){
    ...
    //set_minas(quantidade_linha, quantidade_coluna, quantidade_mina);
...
function checar_vitoria(){
    if(celulas_verificadas.length == linhas * colunas - minas_achadas.length){
        fim_de_jogo = true;
        alert("FIm DE JOGO, Você venceu\nTempo de jogo: XX:XX:XX\nDepois de clicar em ok, o campo vai ser limpo em 3 segundos");
...
function checar_celula(linha, coluna){
    if (linha < 0 || linha >= linhas || coluna < 0 || coluna >= colunas) {
        ...
        minas_achadas += 1;
        console.log(minas_achadas+"minas\n");
    }
}
